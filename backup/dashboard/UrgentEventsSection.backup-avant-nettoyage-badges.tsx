...
const UrgentEventsSection = ({
  ...
  const getFloristStats = (event: Event) => {
    ...
    return {
      ...
      isUrgent: missing > 0 || event.status === EventStatus.DRAFT
    }
  }
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-3">
            ...
            {totalUrgentCount > 3 && (
              ...
              >
                {showMoreUrgent ? 'Voir moins' : `Voir plus (${Math.min(totalUrgentCount - 3, 3)} autres)`}
              </Button>
            )}
          ...
          <div className="space-y-6">
            {(() => {
              ...
              const urgentEvents_real = urgentEvents.filter(event => {
                ...
                // URGENCE = fleuristes non confirmés OU statut à planifier
            ...
            {totalUrgentCount > 3 && (
              <div className="text-center pt-6">
                ...
                >
                  {showMoreUrgent ? 'Voir moins d\'événements' : `Voir plus d'événements (${Math.min(totalUrgentCount - 3, 3)} de plus)`}
                </Button>
              </div>
            )}
          </div>
